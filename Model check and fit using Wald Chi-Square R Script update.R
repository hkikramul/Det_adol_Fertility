...
# 1. Run univariate survey-weighted logistic regressions
uv_results <- lapply(predictors, function(v) {
  ...
  pval <- summary(fit_uv)$coef[2, "Pr(>|t|)"]  # Wald p-value
  data.frame(variable = v, wald_p = pval)
})

# 2. Combine results into one data frame
uv_df <- do.call(rbind, uv_results)

# 3. Format p-values to four decimal places
uv_df <- uv_df %>%
  mutate(wald_p_4dig = sprintf("%.4f", wald_p))
